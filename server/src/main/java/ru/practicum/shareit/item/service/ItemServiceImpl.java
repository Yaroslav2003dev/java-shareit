...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(Long userId, NewItemRequest newItemRequest) {
        ...
        if (newItemRequest.getRequestId() != null) {
            request = requestRepository.findById(newItemRequest.getRequestId()).orElseThrow(() -> new NotFoundException("Запрос не найден"));
        }
        Item item = ItemMapper.toItem(newItemRequest, request);
        if (userId == null) {
